...
export const assignExamToStudent = async (req: Request, res: Response) => {
  try {
    ...
    if (isNaN(examId) || !studentId) {
      return res.status(400).json({ success: false, message: "examId and studentId required" });
    }
    ...
    return res.status(201).json({ success: true, message: "Exam assigned successfully", data: assignment });
  } catch (error: any) {
    // Duplicate assign
    if (error.code === "P2002") {
      return res.status(400).json({ success: false, message: "Exam already assigned to this student" });
    }
    console.error(error);
...
export const bulkAssignExam = async (req: Request, res: Response) => {
  try {
    ...
    if (isNaN(examId) || !studentIds || studentIds.length === 0) {
      return res.status(400).json({ success: false, message: "examId and studentIds required" });
    }
    ...
    if (newIds.length === 0) {
      return res.status(400).json({ success: false, message: "All students already assigned" });
    }
      ...
      include: {
        exam: {
          select: {
            ...
            passingMarks: true, totalMarks: true
          }
        }
      }
    });
