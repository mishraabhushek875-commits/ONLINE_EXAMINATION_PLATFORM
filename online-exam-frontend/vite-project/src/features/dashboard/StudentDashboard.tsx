...
const StatCard = ({
  ...
  <div className="relative overflow-hidden rounded-2xl border border-white bg-white/80 p-4 sm:p-5 shadow-sm backdrop-blur-xl">
    <div className="flex items-center justify-between gap-2">
      ...
      <div className={`rounded-xl p-2.5 sm:p-3 flex-shrink-0 ${gradient}`}>
        {icon}
      </div>
    </div>
  </div>
...
const LatestResultBanner = ({
  ...
  return (
    <div
      className={`mb-6 rounded-3xl border p-5 sm:p-6 shadow-sm backdrop-blur-xl ${
        passed
    ...
    >
      ...
      <div className="flex items-end justify-between gap-4">
        ...
        <div className="relative flex-shrink-0">
          <svg
            className="h-16 w-16 sm:h-20 sm:w-20 -rotate-90"
            viewBox="0 0 36 36"
          >
            <circle
              cx="18"
          ...
          <span
            className={`absolute inset-0 flex items-center justify-center text-base sm:text-lg font-bold ${passed ? "text-emerald-600" : "text-red-500"}`}
          >
            {pct}%
          </span>
        </div>
...
const ExamCard = ({
  ...
  return (
    <div className="rounded-3xl border border-white bg-white/80 p-5 sm:p-6 shadow-sm backdrop-blur-xl transition-all duration-300 hover:shadow-lg hover:shadow-green-400">
      <div className="mb-4 flex items-start justify-between gap-3">
        <h3 className="font-bold text-xl sm:text-2xl">{exam.title}</h3>
        <span
      ...
      <div className="mb-5 grid grid-cols-3 gap-2">
        <div className="rounded-xl bg-blue-50 p-2.5 sm:p-3 text-center">
          <p className="text-sm sm:text-base font-bold text-blue-600">
            {exam.duration}m
          </p>
          <p className="text-xs text-gray-400">Duration</p>
        </div>
        <div className="rounded-xl bg-emerald-50 p-2.5 sm:p-3 text-center">
          <p className="text-sm sm:text-base font-bold text-emerald-600">
            {exam.totalMarks}
          </p>
          <p className="text-xs text-gray-400">Total Marks</p>
        </div>
        <div className="rounded-xl bg-orange-50 p-2.5 sm:p-3 text-center">
          {attemptStatus === "submitted" ? (
            <>
              <p
                className={`text-sm sm:text-base font-bold ${passed ? "text-emerald-600" : "text-red-500"}`}
              >
                {score}
              </p>
...
const StudentDashboard = () => {
  ...
  return (
    <div className="relative overflow-hidden">
      <div className="pointer-events-none absolute -top-40 -left-40 h-[420px] w-[420px] rounded-full bg-blue-500/20 blur-[140px]" />
      <div className="pointer-events-none absolute top-20 -right-32 h-[350px] w-[350px] rounded-full bg-emerald-500/20 blur-[140px]" />
      <div className="pointer-events-none absolute bottom-0 left-1/3 h-[320px] w-[320px] rounded-full bg-orange-500/15 blur-[140px]" />
      <div className="pointer-events-none absolute bottom-20 right-1/4 h-[250px] w-[250px] rounded-full bg-red-500/15 blur-[120px]" />

      <div className="relative z-10 mx-auto max-w-5xl px-4 sm:px-6 pb-20 pt-6 sm:pt-8">
        ...
        {(data?.latestResults?.length ?? 0) > 0 && (
          <div>
            ...
            <div className="overflow-x-auto rounded-3xl border border-white bg-white/80 shadow-sm backdrop-blur-xl">
              <table className="w-full text-sm">
                <thead className="border-b border-gray-100 bg-gray-50/80">
                  <tr>
                    ...
                    <th className="px-4 sm:px-6 py-3 text-right font-semibold text-gray-600">
                      Date
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {data!.latestResults.map((r) => (
                    ...
                    >
                      ...
                      <td className="px-4 sm:px-6 py-4 text-right text-gray-400">
                        {r.submittedAt
